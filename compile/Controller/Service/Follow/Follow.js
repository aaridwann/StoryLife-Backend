...
const follow = async (req, res) => {
    ...
    if (req.user._id == req.query.id)
        return res.status(400).json({ state: false, message: 'id target params cannot same with id user' });
    // 1. Check following
    let check = await (0, exports.checkFollow)(req.user._id, req.query.id);
    if (!check.state) {
        return res.status(400).json(check.message);
        // return failState(check.message)
    }
    // 2. Add to Following
    let addFollow = await (0, exports.addToFollow)(req.user._id, req.query.id);
    if (!addFollow.state) {
        return res.status(500).json(addFollow.message);
        // return failState(addFollow.message)
    }
    // 3. Add to Follower
    let addFollower = await (0, exports.addToFollower)(req.user._id, req.query.id);
    if (!addFollower.state) {
        // if Add to follower is failed do Cancel Following 
        await (0, exports.cancelFollowing)(req.user._id, req.query.id);
