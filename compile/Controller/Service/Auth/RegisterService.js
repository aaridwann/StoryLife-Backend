...
const abortSuspense_1 = __importDefault(require("../Abort Document/abortSuspense"));
...
const CreateSuspenseAccount_1 = __importDefault(require("../Create Document/CreateSuspenseAccount"));
...
const validationRequest = async (req) => {
    ...
    if (password !== confirmationPassword)
        return { state: false, message: 'password not same with confirmation password' };
...
const CreateAdditionalDb = async (email, username) => {
    ...
    let suspense = await (0, CreateSuspenseAccount_1.default)(id);
...
const abortRegister = async (id) => {
    ...
    let suspense = await (0, abortSuspense_1.default)(id);
